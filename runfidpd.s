...
FORWARD INTEGER PROC FNFileRunProgramDropboxPdfB( STRING s1 )
...
PROC Main()
 ...
 //
 AddLine( "g:\dropbox\" )
 AddLine( "g:\mydownloadfiles\from_job_to_home\" )
 AddLine( "d:\mydownloadfiles\" )
 AddLine( "h:\mydownloadfiles\" )
 AddLine( "z:\mydownloadfiles\" )
 //
 ...
 PopPosition()
 //
 Message( FNFileRunProgramDropboxPdfB( s1 ) ) // gives e.g. TRUE
END
...
// library: file: run: program: dropbox: pdf <description></description> <version control></version control> <version>1.0.0.0.46</version> <version control></version control> (filenamemacro=runfidpd.s) [<Program>] [<Research>] [kn, ri, su, 23-01-2022 22:19:38]
INTEGER PROC FNFileRunProgramDropboxPdfB( STRING inS )
 // e.g. PROC Main()
 // e.g.  //
 // e.g.  STRING s1[255] = ""
 // e.g.  //
 ...
 // e.g.  //
 // e.g.  AddLine( "g:\dropbox\" )
 // e.g.  AddLine( "g:\mydownloadfiles\from_job_to_home\" )
 // e.g.  AddLine( "d:\mydownloadfiles\" )
 // e.g.  AddLine( "h:\mydownloadfiles\" )
 // e.g.  AddLine( "z:\mydownloadfiles\" )
 // e.g.  //
 ...
 // e.g.  PopPosition()
 // e.g.  //
 // e.g.  Message( FNFileRunProgramDropboxPdfB( s1 ) ) // gives e.g. TRUE
 // e.g. END
 // e.g.
 // e.g. <F12> Main()
 //
 ...
 PopPosition()
 //
 ExecMacro( Format( "capture", " ", "dir", " ", QuotePath( s ), "*.pdf" ) )
 //
 IF NOT StrFind( "*capture buffer*", CurrFilename(), "i" )
  Warn( "could not go to buffer *capture buffer*" )
  B = FALSE
  PopBlock()
  PopPosition()
  RETURN( B )
 ...
 GotoBlockBegin()
 //
 // clean up: delete all lines not containing '.pdf'
 //
 WHILE ( ( IsCursorInBlock() ) AND ( downB ) )
  //
  IF NOT LFind( ".pdf", "cgi" )
   DelLine()
  ELSEIF NOT LFind( "^{[0-9]|[ ]}[0-9]", "cgix" )
   DelLine()
  ELSE
   downB = Down()
  ENDIF
 ...
 // clean up
 //
 IF StrFind( "*capture buffer*", CurrFilename(), "i" )
  AbandonFile()
 ENDIF
 //
 GotoBufferId( bufferI )
 ...
 BegFile()
 PROCMacroRunKeep( "setwiyde" ) // operation: set: window: warn/yesno: position: x: y: default // new
 IF NOT List( Format( "choose a pdf", " ", caseS ), 120 )
  GotoBufferId( bufferI )
  AbandonFile()
  PopPosition()
  B = FALSE
 ...
 ENDIF
 //
 // caseS = Trim( GetText( 36, 255 ) ) // new [kn, ri, th, 21-04-2022 19:05:57] // new not sure why that was introduced (=36) [kn, ri, mo, 16-01-2023 22:02:15]
 caseS = Trim( GetText( 1, MAXSTRINGLEN ) ) // old [kn, ri, th, 21-04-2022 19:05:52] // reintroduced this again, that works [kn, ri, mo, 16-01-2023 22:02:40]
 //
 // StartPgm( Format( s, caseS ) ) // old [kn, ri, mo, 02-03-2026 19:13:17]
 s = QuotePath( Format( s, caseS ) )
 PROCFileRun4NtAliasCommandListUser( Format( "acrobat", " ", s ) ) // new [kn, ri, mo, 02-03-2026 19:13:52]
 //
 PopPosition()
 PopBlock()
 //
