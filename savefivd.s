// savefivd.s
// Version 2.17 - 2026-02-12 - GitHub PAT push + robust git.exe resolution (fixes Cygwin DLL error)
...
INTEGER PROC FNFileSetUploadGithubFileVersionControlB( STRING yourLocalDirectoryS, STRING githubRemoteDirectoryUrlS, STRING fileNameExecutableGitS, STRING fileNameExecutableTccS, STRING githubUserNameS, STRING githubPasswordS )
 ...
 //
 AddLine( "@echo off" )
 AddLine( "setlocal EnableExtensions EnableDelayedExpansion" )
 AddLine( "" )
 AddLine( "REM --- Resolve a working git.exe ---" )
 AddLine( Format( 'set "GIT_CMD=', fileNameExecutableGitS, '"' ) )
 AddLine( 'if not defined GIT_CMD (' )
 // AddLine( '  for /f "delims=" %%G in ('where git 2^>nul') do ( set "GIT_CMD=%%G" & goto :got_git )' )
 AddLine( Format( '  for /f "delims=" %%G in (', "'", "where git 2^>nul", "'", ') do ( set "GIT_CMD=%%G" & goto :got_git )' ) )
 AddLine( ')' )
 AddLine( ':got_git' )
 AddLine( 'if not defined GIT_CMD (' )
 AddLine( '  echo ERROR: git.exe not found. Install Git for Windows (recommended) or ensure git is on PATH.' )
 AddLine( '  pause' )
 AddLine( '  exit /b 1' )
 AddLine( ')' )
 AddLine( "" )
 AddLine( "REM If a Cygwin git-core helper path is used, switch to Cygwin bin\git.exe and ensure cygwin1.dll is on PATH." )
 AddLine( 'set "GIT_CMD=!GIT_CMD:/=\\!"' )
 AddLine( 'if /i not "!GIT_CMD:\\usr\\libexec\\git-core\\git.exe=!"=="!GIT_CMD!" (' )
 AddLine( '  set "CYGROOT=!GIT_CMD:\\usr\\libexec\\git-core\\git.exe=!"' )
 AddLine( '  set "PATH=!CYGROOT!\\bin;!PATH!"' )
 AddLine( '  set "GIT_CMD=!CYGROOT!\\bin\\git.exe"' )
 AddLine( ')' )
 AddLine( "" )
 ...
 AddLine( "  echo ERROR: GITHUB_PAT is empty. Set it in your TSE ini (or edit the generated .bat) and try again." )
 AddLine( "  pause" )
 AddLine( "  exit /b 1" )
 AddLine( ")" )
 AddLine( "" )
 AddLine( "REM Build an authenticated push URL without leaving the token in .git/config." )
 AddLine( 'set "REMOTE_NOAUTH=%GITHUB_REMOTE%"' )
 AddLine( 'if /i "!REMOTE_NOAUTH:~0,8!"=="https://" set "REMOTE_NOAUTH=!REMOTE_NOAUTH:~8!"' )
 AddLine( 'if /i not "!REMOTE_NOAUTH:~-4!"==".git" set "REMOTE_NOAUTH=!REMOTE_NOAUTH!.git"' )
 AddLine( 'set "REMOTE_AUTH=https://%GITHUB_USER%:%GITHUB_PAT%@!REMOTE_NOAUTH!"' )
 AddLine( "" )
 AddLine( Format( "cdd", " ", yourLocalDirectoryS ) )
 AddLine( "" )
 AddLine( "REM Initialize repo if needed." )
 AddLine( Format( '"%GIT_CMD%"', " ", "rev-parse", " ", "--git-dir", " ", "1>nul", " ", "2>nul" ) )
 AddLine( 'if errorlevel 1 "%GIT_CMD%" init .' )
 AddLine( "" )
 AddLine( "REM Stage + commit changes." )
 AddLine( Format( '"%GIT_CMD%"', " ", "add", " ", "." ) )
 AddLine( Format( '"%GIT_CMD%"', " ", "commit", " ", "-m", " ", commitInformationS ) )
 AddLine( "" )
 AddLine( "REM Ensure branch name is main (GitHub default)." )
 AddLine( Format( '"%GIT_CMD%"', " ", "branch", " ", "-M", " ", "main" ) )
 AddLine( "" )
 AddLine( "REM Configure remote origin." )
 AddLine( Format( '"%GIT_CMD%"', " ", "remote", " ", "get-url", " ", "origin", " ", "1>nul", " ", "2>nul" ) )
 AddLine( 'if errorlevel 1 "%GIT_CMD%" remote add origin "%GITHUB_REMOTE%"' )
 AddLine( "" )
 AddLine( "REM Temporarily set an authenticated URL for push." )
 AddLine( Format( '"%GIT_CMD%"', " ", "remote", " ", "set-url", " ", "origin", " ", '"', "%REMOTE_AUTH%", '"' ) )
 AddLine( "" )
 AddLine( "" )
 AddLine( "REM Determine the remote branch to push to (uses current upstream if set, e.g. origin/TRUNK)." )
 AddLine( 'set "PUSH_REMOTE=origin"' )
 AddLine( 'set "PUSH_BRANCH=main"' )
 // AddLine( 'for /f "delims=" %%U in ('"%GIT_CMD%" rev-parse --abbrev-ref --symbolic-full-name @{u} 2^>nul') do set "UPSTREAM=%%U"' )
 AddLine( Format( 'for /f "delims=" %%U in (', "'", "%GIT_CMD%", "rev-parse --abbrev-ref --symbolic-full-name @{u} 2^>nul", "'", ') do set "UPSTREAM=%%U"' ) )
 AddLine( 'if defined UPSTREAM (' )
 AddLine( '  for /f "tokens=1,2 delims=/" %%A in ("!UPSTREAM!") do ( set "PUSH_REMOTE=%%A" & set "PUSH_BRANCH=%%B" )' )
 AddLine( ')' )
 AddLine( "REM Push to GitHub." )
 AddLine( '"%GIT_CMD%" push -u !PUSH_REMOTE! HEAD:!PUSH_BRANCH!' )
 AddLine( "" )
 AddLine( "REM Restore remote URL without token." )
 AddLine( Format( '"%GIT_CMD%"', " ", "remote", " ", "set-url", " ", "origin", " ", '"', "%GITHUB_REMOTE%", '"' ) )
 AddLine( "" )
 AddLine( "pause" )
 //
