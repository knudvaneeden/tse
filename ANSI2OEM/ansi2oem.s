/*
  ...
  Website        https://ecarlo.nl/tse/ToolsAndExtensions.html#Ansi2oem
  Version        v3.0.1   25 Oct 2024
  ...
  INSTALLATION
    This extension only works for the GUI versions of TSE (g32.exe).
...
proc save_settings(integer specifically)
  ...
  if not is_autoloaded()
    if not AddAutoLoadMacro(SplitPath(CurrMacroFilename(), _NAME_))
      Alarm()
      Warn('Could not add macro "', SplitPath(CurrMacroFilename(), _NAME_),
           '" to file "tseload.dat".')
    endif
  endif
end
...
proc un_install()
  ...
  if YesNo("Really uninstall Ansi2oem?") == 1
    ...
    if not DelAutoLoadMacro(SplitPath(CurrMacroFilename(), _NAME_))
      Alarm()
      Warn('Could not delete macro "', SplitPath(CurrMacroFilename(), _NAME_),
           '" from file "tseload.dat".')
    endif
