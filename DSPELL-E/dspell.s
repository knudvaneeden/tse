...
//	Portable package version 1.0.0.0.7 - 2026-09-05
//	DSPELL2.MAC is loaded relative to DSPELL.MAC.
//	Asks before checking a whole file larger than 100 KiB.
//	Contains built-in DSpell help; GETHELP.MAC is no longer required.
...
constant LARGE_FILE_SIZE = 102400

string bm_ORIGIN[] = "95"
string bm_BEGIN[] = "96"
string bm_END[] = "97"

string dspell2[] = "dspell2"
string dspell2File[255] = ""
...
proc HelpMe(string topic)
	...
	integer helpBufferI = CreateTempBuffer()

	if not helpBufferI
		Warn("DSpell: Unable to create the help buffer.")
		return()
	endif
	...
	if (topic == "Edit Words")
		AddLine("EDIT WORDS")
		AddLine("")
		AddLine("This window edits a DSpell dictionary or flagged-word list.")
		AddLine("")
		AddLine("Ins       Insert a word.")
		AddLine("Del       Delete the selected word.")
		AddLine("Esc       Finish editing and return.")
		AddLine("F1        Display this help.")
		AddLine("")
		AddLine("Changed word lists are sorted before they are saved.")
		AddLine("USER.LEX contains accepted user words.")
		AddLine("FLAG.LEX contains words that must always be flagged.")
	elseif (topic == "Managing Custom Dictionaries")
		AddLine("MANAGING CUSTOM DICTIONARIES")
		AddLine("")
		AddLine("Ins       Add an existing dictionary file.")
		AddLine("Del       Remove the selected dictionary from DSpell.")
		AddLine("Enter     Edit the words in the selected dictionary.")
		AddLine("Space     Enable or disable the selected dictionary.")
		AddLine("Esc       Finish and return.")
		AddLine("F1        Display this help.")
		AddLine("")
		AddLine("[X] means that a custom dictionary is enabled.")
		AddLine("[ ] means that it is present but disabled.")
		AddLine("Large custom dictionaries can make checking slower.")
	else
		AddLine("DSPELL HELP")
		AddLine("")
		AddLine("DSpell checks a file, the rest of a file, a marked block,")
		AddLine("or the word under the cursor. AutoSpell can check words")
		AddLine("after Space, Enter, and optionally Tab while you type.")
		AddLine("")
		AddLine("SPELLCHECK WINDOW")
		AddLine("Up/Down   Select a misspelled word.")
		AddLine("Left/Right Select the previous or next occurrence.")
		AddLine("A         Add the word to USER.LEX.")
		AddLine("C/Enter   Change all occurrences.")
		AddLine("Shift+C or Ctrl+Enter changes only this occurrence.")
		AddLine("I/Del     Ignore the word for this session.")
		AddLine("O         Open DSpell options.")
		AddLine("F7/Tab    Edit the document or resume checking.")
		AddLine("Esc       Finish the spelling session.")
		AddLine("")
		AddLine("REPLACEMENT PROMPT")
		AddLine("Type a replacement, or press Up for spelling suggestions.")
		AddLine("Enter accepts the replacement; Escape cancels it.")
		AddLine("")
		AddLine("COMMAND-LINE MODES")
		AddLine("-f file   -r rest   -b block   -w word")
		AddLine("-m menu   -o options   no option: automatic mode")
		AddLine("")
		AddLine("AUTOSPELL")
		AddLine("Ctrl+Shift+A temporarily overrides AutoSpell on or off.")
		AddLine("An S on the status line indicates that AutoSpell is active.")
		AddLine("Disable Tab checking if it conflicts with TEMPLATE.MAC.")
		AddLine("")
		AddLine("DICTIONARIES")
		AddLine("SEMWARE.LEX is the required main dictionary.")
		AddLine("USER.LEX stores accepted words; PERSONAL.LEX ignores words.")
		AddLine("FLAG.LEX forces listed words to be reported as misspelled.")
	endif

	BegFile()
	FileChanged(FALSE)
	List("DSpell Help - " + topic, 78)
	GotoBufferId(originalBufferI)
	AbandonFile(helpBufferI)
end
...
proc Save(integer fForceSave)
	// save settings
	WriteProfileInt(section, st_AutoSpell, g_fAutoSpell)
	WriteProfileInt(section, st_BeepWord, g_fBeepWord)
	WriteProfileInt(section, st_FlashWord, g_fFlashWord)
	WriteProfileInt(section, st_TabKey, g_fTabKey)
	WriteProfileInt(section, st_IgnoreNonAlphaWords, g_fIgnoreNonAlphaWords)
	WriteProfileInt(section, st_VerifyNewWord, g_fVerifyNewWord)
	WriteProfileStr(section, st_NoAutoSpellExts, noautospellExt)
	#ifdef WIN32
	WriteProfileStr(section, st_MisspelledSound, g_stMisspelled)
	#endif

	WriteProfileInt(section, st_Text, g_nText)
	WriteProfileInt(section, st_Hilite, g_nHilite)
...
proc SaveAddlWordList()
	...
	if id_dict and GotoBufferId(id_dict) and FileChanged()
		WriteProfileInt(section, stNumDictionaries, NumLines())
		BegFile()
		for i = 1 to NumLines()
			BegLine()
			WriteProfileStr(section, "Dictionary"+Str(i),
					iif(CurrChar() == Asc("*"), "*", "") +
					GetToken(GetText(2, 255), " ", 2))
			Down()
		endfor
		FileChanged(FALSE)
	endif
...
integer proc ChangeWord(integer fAll)
	...
	// Suggestions use the built-in replacement history.  Do not call
	// DelHistory() for a built-in history list.
	//GotoWindow(DocumentWindow)
	//UpdateDisplay(_ALL_WINDOWS_REFRESH_)
	GotoWindow(WordListWindow)
	return (fRet)
...
integer proc Load2()
	...
	// Load the companion macro from the directory containing DSPELL.MAC.
	// This makes DSpell independent of the current working directory and
	// removes the requirement to add the package directory to TSEPATH.
	dspell2File = SplitPath(CurrMacroFilename(), _DRIVE_|_PATH_) + "dspell2.mac"
	f = isMacroLoaded(dspell2) or LoadMacro(dspell2File)
...
proc WhenLoaded()
	...
	// load settings
	g_fAutoSpell = GetProfileInt(section, st_AutoSpell, TRUE)
	g_fBeepWord = GetProfileInt(section, st_BeepWord, TRUE)
	g_fFlashWord = GetProfileInt(section, st_FlashWord, TRUE)
	g_fTabKey = GetProfileInt(section, st_TabKey, TRUE)
	g_fIgnoreNonAlphaWords = GetProfileInt(section, st_IgnoreNonAlphaWords, TRUE)
	g_fVerifyNewWord = GetProfileInt(section, st_VerifyNewWord, TRUE)
	noautospellExt = GetProfileStr(section, st_NoAutoSpellExts, noautospellExt)
	#ifdef WIN32
	g_stMisspelled = GetProfileStr(section, st_MisspelledSound, "")
	#endif
	g_nText = GetProfileInt(section, st_Text, Query(MenuTextAttr))
	g_nHilite = GetProfileInt(section, st_Hilite, Query(HiLiteAttr))
	...
	c = GetProfileInt(section, stNumDictionaries, 0)
	cid = GetBufferId()
	id_dict = CreateTempBuffer()
	if id_dict
		for i = 1 to c
			fn = GetProfileStr(section, "Dictionary"+Str(i), "")
			if fn[1] == "*"
				ch = fn[1]
				fn = fn[2:255]
			else
...
integer proc FNIsLargeFile()
	integer fileSizeI = 0
	integer lineI

	PushPosition()
	BegFile()
	for lineI = 1 to NumLines()
		fileSizeI = fileSizeI + CurrLineLen() + 2
		if (fileSizeI > LARGE_FILE_SIZE)
			PopPosition()
			return(TRUE)
		endif
		Down()
...
proc Spell(integer nFlavor)
	...
	// A whole-file check can take a long time.  CurrLineLen() is used
	// because FileSize() is not a TSE SAL function.  Stop counting as
	// soon as the buffer is known to be larger than 100 KiB.
	if (nFlavor == SPELL_FILE) and FNIsLargeFile()
		if (YesNo("This file is larger than 100 KB. Spellchecking may take a long time. Continue?") <> 1)
			return()
		endif
	endif
