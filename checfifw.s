...
PROC Main()
 ...
 STRING s1[255] = FNStringGetFilenameOnlyCurrentS() // change this
 STRING s2[255] = "G:\VERSIONCONTROL\SUBVERSION\W1\" // change this
 PROCMacroRunKeep( "setwiyde" ) // operation: set: window: warn/yesno: position: x: y: default // new
 IF ( NOT ( Ask( "file: check: version: control: stored: fileNameS = ", s1, _EDIT_HISTORY_ ) ) AND ( Length( s1 ) > 0 ) ) RETURN() ENDIF
 PROCMacroRunKeep( "setwiyde" ) // operation: set: window: warn/yesno: position: x: y: default // new
 IF ( NOT ( Ask( "file: check: version: control: stored: directoryVersionControlS = ", s2, _EDIT_HISTORY_ ) ) AND ( Length( s2 ) > 0 ) ) RETURN() ENDIF
 Message( FNFileCheckVersionControlStoredB( s1, s2 ) ) // gives e.g. TRUE
END
