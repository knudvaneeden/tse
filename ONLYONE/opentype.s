...
// Read the optional startup-message setting from the current directory.
integer proc silent_startup()
   integer original_id = GetBufferId()
   integer config_id
   integer line_number
   integer result = FALSE
   string setting [255]
   if FileExists("opentype.ini")
      config_id = CreateTempBuffer()
      GotoBufferId(config_id)
      if LoadBuffer("opentype.ini")
         for line_number = 1 to NumLines()
            GotoLine(line_number)
            setting = Trim(GetText(1, 255))
            if setting == "silent=true"
               result = TRUE
            elseif setting == "silent=false"
               result = FALSE
            endif
         endfor
      endif
...
proc Main()
   if not silent_startup()
      Warn("OpenType 1.3: Configure the Windows File Open type list. "
           + "Requires UseCommonDialogs to be ON; configuration follows.")
   endif
   configure()
end
