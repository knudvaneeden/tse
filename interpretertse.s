...
FORWARD INTEGER PROC FNProgramRunInterpreterTseB( STRING s1 )
...
PROC Main()
 ...
 STRING s1[255] = "loadaddjobdok" // change this
 PROCMacroRunKeep( "setwiyde" ) // operation: set: window: warn/yesno: position: x: y: default // new
 // IF ( NOT ( Ask( "program: run: interpreter: tse: commandS = ", s1, _EDIT_HISTORY_ ) ) AND ( Length( s1 ) > 0 ) ) RETURN() ENDIF
 Message( FNProgramRunInterpreterTseB( s1 ) ) // gives e.g. TRUE
END
...
// library: program: run: interpreter: tse <description></description> <version control></version control> <version>1.0.0.0.4</version> <version control></version control> (filenamemacro=interpretertse.s) [<Program>] [<Research>] [kn, ri, su, 19-04-2026 22:59:38]
INTEGER PROC FNProgramRunInterpreterTseB( STRING commandS )
 // e.g. PROC Main()
 // e.g.  // STRING s1[255] = GetHistoryStr( _EDIT_HISTORY_, 1 ) // change this
 // e.g.  STRING s1[255] = "loadaddjobdok" // change this
 // e.g.  PROCMacroRunKeep( "setwiyde" ) // operation: set: window: warn/yesno: position: x: y: default // new
 // e.g.  // IF ( NOT ( Ask( "program: run: interpreter: tse: commandS = ", s1, _EDIT_HISTORY_ ) ) AND ( Length( s1 ) > 0 ) ) RETURN() ENDIF
 // e.g.  Message( FNProgramRunInterpreterTseB( s1 ) ) // gives e.g. TRUE
 // e.g. END
 // e.g.
 // e.g. <Ctrl F12> Main()
 //
 ...
 */
 ...
 // ===
 //
 // e.g. // QuickHelp( HELPDEFFNProgramRunInterpreterTseB )
 // e.g. HELPDEF HELPDEFFNProgramRunInterpreterTseB
 // e.g.  title = "FNProgramRunInterpreterTseB( s1 ) help" // The help's caption
 // e.g.  x = 100 // Location
 // e.g.  y = 3 // Location
 // e.g.  //
 ...
 STRING caseS[255] = ""
 //
 IF EquiStr( Trim( Query( MacroCmdLine ) ), "" )
  //
  caseS = commandS
  //
 ELSE
  //
  caseS = Query( MacroCmdLine )
  //
 ENDIF
 //
 CASE caseS
  ...
  WHEN "loadaddjobdok"
   //
   PROCMacroRunPurge( "runmadlo" ) // operation: load: file: database: do: local
   //
  WHEN "loadbibtses"
   //
   PROCMacroRunPurge( "keymfits" ) // operation: load: file: library: tse
   //
  OTHERWISE
   //
   Warn( "FNProgramRunInterpreterTseB(", " ", "case", " ", ":", " ", caseS, ": not known" )
   //
   B = FALSE
   //
   RETURN( B )
...
// library: file: check: edit: central: message <description></description> <version control></version control> <version>1.0.0.0.10</version> <version control></version control> (filenamemacro=checfiex.s) [<Program>] [<Research>] [kn, ho, mo, 17-04-2006 17:41:21]
INTEGER PROC FNFileCheckEditCentralMessageB( STRING filenameInS, INTEGER messageB )
 // e.g. PROC Main()
 ...
 STRING s[255] = ""
 //
 STRING fileNameS[255] = fileNameInS
 //
