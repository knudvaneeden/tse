...
// library: file: save: to: directory: local: git: version: control <description></description> <version control></version control> <version>1.0.0.0.5</version> <version control></version control> (filenamemacro=savefivc.s) [<Program>] [<Research>] [kn, ri, su, 18-02-2018 05:40:49]
INTEGER PROC FNFileSaveCurrentToDirectoryLocalGitVersionControlB( STRING yourLocalDirectoryS, STRING githubRemoteDirectoryUrlS, STRING fileNameExecutableGitS, STRING fileNameExecutableTccS, STRING githubUserNameS, STRING githubPasswordS )
 ...
 INTEGER B = FALSE
 //
 STRING fileNameS[255] = Format( yourLocalDirectoryS, SplitPath( CurrFileName(), _NAME_ | _EXT_ ) )
 //
 IF FileExists( fileNameS )
  //
  Warn( "This file", ":", " ", "exists already in your local GitHub directory", ":", yourLocalDirectoryS )
  //
  IF ( FNMathGetNumberInputYesNoCancelPositionDefaultI( "Do you first want to upload this local GitHub directory to the remote GitHub directory" ) == 1 )
   ...
   IF ( NOT ( B ) )
    //
    Warn( "could not upload the local GitHub directory. Please check." )
    //
    B = FALSE
    //
    RETURN( B )
