...
//
string proc GetCurrentExtension()
...
string proc GetIniFileName ()
    return (SplitPath(CurrMacroFileName(), _DRIVE_ | _PATH_) + "mlttab.ini")
end

integer proc SilentMode ()
    ...
    if iniBuffer
        if InsertFile(GetIniFileName())
            if lFind("silent=", "^gi")
                line = GetText(1, 255)
                value = Lower(Trim(SubStr(line, Pos("=", line) + 1, 255)))
                if value == "true"
                    silent = TRUE
                endif
            endif
...
proc SaveDefSettings ()
    if BufferType () == _NORMAL_
        SetLookup(GetCurrentExtension(), Encode_Settings())
    endif
end
...
Menu TabMenu()
    ...
    "&Word wrap"    [iif(Query(WordWrap),"Yes","No"):5],
        Set(WordWrap, iif(Query(WordWrap), FALSE, TRUE)),
        DontClose,
        "Toggle word wrap"
    ...
    "",
        ,
        Divide
    "Set as &Default for type " [GetCurrentExtension(): 4],
        SaveDefSettings(),
        ,
...
proc Main ()
    if not SilentMode()
        Warn("MULTITAB " + GSVersion + " - Configure separate tab, indent, wrap, and margin settings per file or file type. The install menu will open next. Updated with OpenAI GPT-5.6.")
    endif
    InstallMenu()
end
...
proc Init_Settings ()
    ...
    if BufferType() == _NORMAL_
        code = GetLookup(GetCurrentExtension())
        if code <> -1
