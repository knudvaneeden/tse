...
FORWARD INTEGER PROC FNFileRunArtificialIntelligenceCaseB( STRING s1 )
...
PROC Main()
 ...
 STRING s1[255] = "prompt*.*" // change this
 PROCMacroRunKeep( "setwiyde" ) // operation: set: window: warn/yesno: position: x: y: default // new
 IF ( NOT ( AskFilename( "file: run: artificial: intelligence: case: fileNameInS = ", s1, _DEFAULT_, _EDIT_HISTORY_ ) ) AND ( Length( s1 ) > 0 ) ) RETURN() ENDIF
 Warn( FNFileRunArtificialIntelligenceCaseB( s1 ) ) // gives e.g. TRUE
END
...
INTEGER PROC FNFileRunArtificialIntelligenceCaseB( STRING fileNameInS )
 ...
 // e.g.  STRING s1[255] = "prompt*.*" // change this
 // e.g.  PROCMacroRunKeep( "setwiyde" ) // operation: set: window: warn/yesno: position: x: y: default // new
 // e.g.  IF ( NOT ( AskFilename( "file: run: artificial: intelligence: case: fileNameInS = ", s1, _DEFAULT_, _EDIT_HISTORY_ ) ) AND ( Length( s1 ) > 0 ) ) RETURN() ENDIF
 // e.g.  Warn( FNFileRunArtificialIntelligenceCaseB( s1 ) ) // gives e.g. TRUE
 // e.g. END
 // e.g.
 // e.g. <Ctrl F12> Main()
 //
 // ===
 //
 // Use case = Create a prompt created from text in an existing file and paste it in a chosen AI
 //
 // ===
 //
 // ===
 ...
 */
 ...
 // ===
 //
 // e.g. // QuickHelp( HELPDEFFNFileRunArtificialIntelligenceCaseB )
 // e.g. HELPDEF HELPDEFFNFileRunArtificialIntelligenceCaseB
 // e.g.  title = "FNFileRunArtificialIntelligenceCaseB( s1 ) help" // The help's caption
 // e.g.  x = 100 // Location
 // e.g.  y = 3 // Location
 // e.g.  //
 ...
 IF NOT FileExists( fileNameS )
  //
  Warn( fileNameS, ":", " ", "This file can not be found or does not exist. Please make sure you have a filename with a prompt text created." )
  //
  B = FALSE
  //
  PopBlock()
 ...
 IF NOT EditFile( fileNameS )
  Warn( fileNameS, ":", " ", "could not be opened for editing. Please check." )
  //
  B = FALSE
  //
  PopBlock()
 ...
 MarkAll()
 //
 B = FNBlockRunArtificialIntelligenceCaseB()
 //
 PopBlock()
 PopPosition()
 //
