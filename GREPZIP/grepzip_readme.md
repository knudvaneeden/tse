...
Run `GREPZIP` as a macro, or press **Ctrl+Alt+Shift+F**. Press **Ctrl+Alt+Shift+R** to open TSE's built-in **Regular Expression Operators** help topic.

1. Enter the search string.
2. Enter TSE search options, for example `i` or `ix`.
3. Enter a directory, file, or file specification.
4. Wait while GREPZIP enumerates files, expands supported archives, and asks TSE to search each file. The currently searched virtual filename is refreshed on TSE's message bar.
...
Archive members use `::` to show archive nesting, for example:

```text
C:\DATA\outer.zip::docs/inner.zip::source/test.s(42,7): proc Main()
C:\DATA\outer.tgz::source.tar::source/test.s(42,7): proc Main()
```
...
- Encrypted, damaged, or unsupported archives are skipped.
- With a wildcard source specification, the mask selects ordinary files both outside and inside archives; supported archive files are still opened so matching members can be found.
...
- Continues from the stable PowerShell-based version 1.0.0.0.7.
- Adds recursive JAR, TAR, TGZ and TAR.GZ searching.
- Allows supported archive types to be nested in any combination.
- Applies wildcard file specifications to ordinary files inside archives while still following nested archives.
- Refreshes TSE's display after every filename message to prevent the message-bar filename from appearing frozen.
- Changes the direct search key to Ctrl+Alt+Shift+F.
- Versions 1.0.0.0.8 and 1.0.0.0.9 were experimental DLL-enumeration versions and are not the basis of this release.
...
- Direct key binding in that release: Ctrl+Alt+Shift+G.
