...
// Running the compiled macro directly displays this introduction.
// The original Ctrl+1 and Ctrl+2 bindings retain their OS/2 behavior.
proc Main()
   string lineS[255] = ""
   integer silentI = FALSE

   if FileExists("os2cmd.ini")
      PushLocation()
      if EditFile("os2cmd.ini", _dont_prompt_)
         BegFile()
         repeat
            lineS = Lower(GetText(1, 255))
   ...
   if not silentI
      Warn("OS2CMD 1.0.0.0.1: OS/2 Warp only. Ctrl+1 starts a foreground command; Ctrl+2 starts a background command. Requires hstart.exe and C:\config.sys.")
   endif
end
