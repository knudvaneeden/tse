...
integer long_lines_found = FALSE
integer silent_mode = FALSE
...
proc Main()
   ...
   integer line_to   = 0
   integer old_rtw = Set(RemoveTrailingWhite, ON)
   silent_mode = Lower(GetProfileStr("OSQLFMT", "silent", "false", "osqlfmt.ini")) == "true"
   ...
   if not silent_mode
      if long_lines_found
         Warn("OSQLFMT finished formatting this SQL script. Some statements were too long to join; review the result before using it.")
      else
         Warn("OSQLFMT finished formatting this SQL script. Review the result before using it.")
      endif
   endif
end
