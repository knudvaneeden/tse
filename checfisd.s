...
PROC Main()
 ...
 // STRING s1[255] = "G:\VERSIONCONTROL\GIT\DDD01\" // change this
 STRING s1[255] = "G:\VERSIONCONTROL\SUBVERSION\W1\" // change this
 PROCMacroRunKeep( "setwiyde" ) // operation: set: window: warn/yesno: position: x: y: default // new
 IF ( NOT ( Ask( "file: check: version: control: stored: current: directoryVersionControlS = ", s1, _EDIT_HISTORY_ ) ) AND ( Length( s1 ) > 0 ) ) RETURN() ENDIF
 Message( FNFileCheckVersionControlStoredCurrentB( s1 ) ) // gives e.g. TRUE
END
