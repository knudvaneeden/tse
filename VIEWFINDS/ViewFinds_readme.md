# ViewFinds Extension for TSE (v1.1.10)
...
*   **TSE v4.50 to v4.50.24 (including rc versions):** Search-term highlighting fallback using instantaneous lazy evaluation[cite: 3].
...
*   **`list_startup()` / `list_cleanup()`**: Event hooks that trigger when the View Finds window opens or closes. `list_startup` intercepts the window, caches the active search history to prevent redundant engine queries, and triggers the attribute construction[cite: 3].
...
*   **`hd_draw_list_line()`**: Intercepts the screen drawing routine. In older versions, this utilizes **Lazy Evaluation**, dynamically generating string-formatting and stamping search-term highlights onto the string directly as you scroll, completely eliminating buffer-switch bottlenecks[cite: 3].
