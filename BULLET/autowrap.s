...
remmain on the same screen row, allowing you to see several
...
defferent line, change the math.
********************************************/
...
proc OnDelChar()
    ...
    if BufferType() <> _NORMAL_ or NumNonWhite() < 40
    	return()
    endif
...
proc OnSelfInsert()
    ...
    // bail if system or hidden buffer, or if short line, or no wordwrap
    if qw == 0 or BufferType() <> _NORMAL_ or NumNonWhite() < 40
    	return()
    endif
