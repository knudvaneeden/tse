...
If a path is stored in `searchtemplate.ini`, that path is offered. Otherwise, `LoadDir() + "template.dat"` is offered.
...
### 1.0.0.0.5 - 2026-09-19 11:20:28 UTC

- Moved the full path to `template.dat` to the fourth and final `Ask()` input.
- The input order is now abbreviation, options, extension, and template path.
