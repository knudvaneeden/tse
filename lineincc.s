...
FORWARD INTEGER PROC FNTextCheck_SearchExpressionFoundNotB( STRING s1, STRING s2 )
...
FORWARD PROC PROCLineChangeNumberTotalCurrent( STRING s1, STRING s2, STRING s3 )
...
FORWARD STRING PROC FNStringGetCommaInsertS( STRING s1 )
...
FORWARD STRING PROC FNStringGetFileDiskCentralS( STRING s1, STRING s2 )
...
FORWARD STRING PROC FNStringGetFileSizeDiskCommaInsertCurrentS()
FORWARD STRING PROC FNStringGetFileSizeDiskCommaInsertS( STRING s1 )
FORWARD STRING PROC FNStringGetFileSizeDiskS( STRING s1 )
...
PROC Main()
 PROCLineChangeNumberTotalCurrent( "ENG", "-", FNStringGetFilenameCurrentS() )
 // PROCLineChangeNumberTotalCurrent( "NED", "-" )
END
...
// library: line: number: copy current line replace the linenumber information with the current total line numbers. Further date information with 1 working day <description></description> <version control></version control> <version>1.0.0.0.11</version> (filenamemacro=lineincc.s) [<Program>] [<Research>] [kn, zoe, th, 10-08-2000 17:09:57]
PROC PROCLineChangeNumberTotalCurrent( STRING languageS, STRING separatorS, STRING filenameS )
 // e.g. PROC Main()
 // e.g.  PROCLineChangeNumberTotalCurrent( "ENG", "-", FNStringGetFilenameCurrentS() )
 // e.g.  // PROCLineChangeNumberTotalCurrent( "NED", "-" )
 // e.g. END
 // e.g.
 // e.g. <F12> Main()
 //
 STRING s[255] = ""
 //
 EndFile() // new [kn, ri, tu, 03-03-2026 15:06:37]
 //
 PROCCursorGotoDown()
 //
 PROCTextGotoLineBegin()
 //
 IF FNTextCheck_SearchExpressionFoundNotB( "lines", "iw" )
  //
  PROCWarnCons( "no lines found in :", filenameS )
  //
  RETURN()
  //
 ENDIF
 //
 IF FNTextCheck_SearchExpressionFoundNotB( "{[0-9].#}", "xb" )
  //
  PROCWarnCons( "no linenumber found in file:", filenameS )
  //
  RETURN()
  //
 ENDIF
 ...
 PushPosition()
 //
 s =  FNStringGetFileSizeDiskCommaInsertCurrentS()
 //
 IF FNErrorCheckSB( s )
  //
  RETURN()
 ...
 ENDIF
 //
 InsertText( Format( "[", NumLines(), " ", "lines", " ", FNStringGetDateTodayFormatKnudDefaultS(), "]", " ", "[", s, " ", "bytes", "]" ) )
 //
END
...
// library: text: check: search: expression: found: not <description></description> <version control></version control> <version>1.0.0.0.1</version> (filenamemacro=chectefn.s) [<Program>] [<Research>] [kn, ri, sa, 25-08-2001 21:58:29]
INTEGER PROC FNTextCheck_SearchExpressionFoundNotB( STRING searchS, STRING searchOptionS )
 // e.g. PROC Main()
 // e.g.  Message( FNTextCheck_SearchExpressionFoundNotB( "test", "i" ) ) // gives e.g. TRUE when that text is not found
 // e.g. END
 // e.g.
 // e.g. <F12> Main()
 //
 RETURN( FNMathCheckLogicNotB( FNTextCheckSearchExpressionFoundB( searchS, searchOptionS ) ) )
 //
END
...
// library: string: get: file: size: disk: comma: insert <description></description> <version>1.0.0.0.4</version> <version control></version control> (filenamemacro=getstcit.s) [<Program>] [<Research>] [kn, ri, fr, 19-08-2011 01:14:31]
STRING PROC FNStringGetFileSizeDiskCommaInsertCurrentS()
 // e.g. PROC Main()
 // e.g.  Message( FNStringGetFileSizeDiskCommaInsertCurrentS() ) // gives e.g total amount of file size in bytes on disk of current file
 // e.g. END
 // e.g.
 // e.g. <F12> Main()
 //
 RETURN( FNStringGetFileSizeDiskCommaInsertS( CurrFilename() ) )
 //
END
...
// library: string: get: file: size: disk: comma: insert <description></description> <version>1.0.0.0.2</version> <version control></version control> (filenamemacro=getstcit.s) [<Program>] [<Research>] [kn, ri, fr, 19-08-2011 01:14:31]
STRING PROC FNStringGetFileSizeDiskCommaInsertS( STRING fileNameS )
 // e.g. PROC Main()
 // e.g.  Message( FNStringGetFileSizeDiskCommaInsertS( CurrFilename() ) ) // gives e.g total amount of file size in bytes on disk, comma separated
 // e.g. END
 // e.g.
 // e.g. <F12> Main()
 //
 RETURN( FNStringGetCommaInsertS( FNStringGetFileSizeDiskS( fileNameS ) ) )
 //
END
...
// library: string: get: comma: insert <description></description> <version>1.0.0.0.4</version> <author>Semware, InsertCommas(), fileinfo.s</author> <version control></version control> (filenamemacro=getstcis.s) [<Program>] [<Research>] [kn, ri, fr, 19-08-2011 01:07:52]
STRING PROC FNStringGetCommaInsertS( STRING inS )
 // e.g. PROC Main()
 // e.g.  Message( FNStringGetCommaInsertS( "123456789" ) ) // gives e.g. "123,456,789"
 // e.g. END
 // e.g.
 // e.g. <F12> Main()
 //
 STRING s[26] = inS
 //
 INTEGER idxI = ( length( inS ) MOD 3 ) + 1
 //
 IF ( Length( inS ) > 3 )
  //
  WHILE ( idxI < Length( s ) )
...
// library: string: get: file: size: disk <description></description> <version>1.0.0.0.3</version> <version control></version control> (filenamemacro=getstsdr.s) [<Program>] [<Research>] [kn, ri, fr, 19-08-2011 01:11:39]
STRING PROC FNStringGetFileSizeDiskS( STRING fileNameS )
 // e.g. PROC Main()
 // e.g.  Message( FNStringGetFileSizeDiskS( CurrFilename() ) ) // gives e.g total amount of file size in bytes on disk
 // e.g. END
 // e.g.
 // e.g. <F12> Main()
 //
 RETURN( FNStringGetFileDiskCentralS( fileNameS, "ffsize" ) )
 //
END
...
// library: string: get: file: disk: central <description></description> <version>1.0.0.0.3</version> <version control></version control> (filenamemacro=getstdce.s) [<Program>] [<Research>] [kn, ri, sa, 20-08-2011 17:05:24]
STRING PROC FNStringGetFileDiskCentralS( STRING fileNameS, STRING caseS )
 // e.g. PROC Main()
 // e.g.  Message( FNStringGetFileDiskCentralS( CurrFilename(), "ffdate" ) ) // gives e.g total amount of file size in bytes on disk
 // e.g. END
 // e.g.
 // e.g. <F12> Main()
 //
 ...
 PushPosition()
 //
 IF ( NOT ( FindThisFile( fileNameS, -1 ) ) )
  //
  Warn( "File", ":", " ", fileNameS, " ", "not found" )
   //
   PopPosition()
   //
 ...
 CASE caseS
  ...
  OTHERWISE
   //
   Warn( "case", ":", " ", caseS, " ", "not found in", " ", "FNStringGetFileDiskCentralS(" )
   //
   s = FNStringGetErrorS()
   //
 ENDCASE
