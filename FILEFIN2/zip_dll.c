/* FILEFIN2 archive reader for TSE 4.50.
   ...
   Version 1.0.0.0.30 - 2026-09-18 - OpenAI Codex */
...
static int is_archive_name(const char *name)
{
    return has_extension(name, ".zip") || has_extension(name, ".jar") ||
           ...
           has_extension(name, ".rar") || has_extension(name, ".7z");
}

static int is_helper_archive_name(const char *name)
{
...
__declspec(dllexport) int PASCAL ZIP_Open(SAL_STRING *pathS, SAL_STRING *stateS)
{
    ...
    if (!find_central_directory(file, size, &offset, &entries) || !seek64(file, offset)) {
        ...
        if (!is_helper_archive_name(path)) return 0;
        entries = 0;
    }
    ...
    contexts[slot].hasNested = is_helper_archive_name(path);
