...
FORWARD STRING PROC FNStringGetDirectoryVersionControlSubversionRepositoryS()
...
PROC Main()
 ...
 STRING s1[255] = FNStringGetDirectoryVersionControlSubversionWorkingS()
 //
 // STRING s2[255] = "C:\TEMP\R1\"
 STRING s2[255] = FNStringGetDirectoryVersionControlSubversionRepositoryS()
 //
 ...
 STRING s3[255] = ""
 //
 // STRING s4[255] = "C:\TEMP\the-semware-editor-tse-code\TRUNK\" // old [kn, ri, th, 12-02-2026 19:05:43]
 STRING s4[255] = "G:\VERSIONCONTROL\SUBVERSION\the-semware-editor-tse-code\TRUNK\" // new [kn, ri, th, 12-02-2026 19:10:59]
 //
 ...
 IF LFind( Format( "(filenamemacro=", s0, ")" ), "gi" )
  ...
  ENDIF
 ENDIF
 // s3 = Format( "[", s0, "]", s, "recompile|draft|backup|works|created|add setm|replace menu hotkey|save|major|minor|compile|refactor|original" ) // old [kn, zoe, mo, 20-11-2000 14:31:57]
 // s3 = Format( "recompile|added|changed|error|warn|backup|created|works|replace menu hotkey|save|major|minor|compile|refactor|original|add setm" ) // old [kn, ri, su, 10-11-2024 15:49:05]
 // s3 = Format( "corrected:changed:added:recompile:backup:created:works:error:warn:replace menu hotkey:save:major:minor:compile:refactor:original:add setm" ) // new [kn, ri, su, 10-11-2024 15:53:57]
 s3 = FNStringGetInitializationSubversionS() // new [kn, ri, tu, 31-12-2024 18:12:11]
 PopBlock()
 PopPosition()
 //
 PushPosition()
...
// e.g   PushKey( <Home> )
 PROCMacroRunKeep( "setwiyde" ) // operation: set: window: warn/yesno: position: x: y: default // new
 // IF ( NOT ( Ask( "file: save: version: control: subversion: revisionChangeInformationS = ", s3, _EDIT_HISTORY_ ) ) AND ( Length( s3 ) > 0 ) ) RETURN() ENDIF // old [kn, ri, fr, 17-05-2024 16:11:35]
 IF ( NOT ( Ask( Format( "[", s0, "]", s, "file: save: version: control: subversion: revisionChangeInformationS = " ), s3, _EDIT_HISTORY_ ) ) AND ( Length( s3 ) > 0 ) ) RETURN() ENDIF // new [kn, ri, fr, 17-05-2024 16:11:40]
 s3 = Format( "[", s0, "]", s, s3 ) // new [kn, ri, fr, 17-05-2024 16:11:15]
 s3 = StrReplace( '"', s3, "'", "" ) // make sure no double quotes are present as this overrules the outer double quote and will cause an 'svn out of date' error.
 //
 IF ( Length( s3 ) > Val( s5 ) )
...
// library: string: get: directory: version: control: repository <description></description> <version>1.0.0.0.4</version> <version control></version control> (filenamemacro=getstcrg.s) [<Program>] [<Research>] [kn, am, mo, 11-10-2010 10:18:56]
...
// library: string: get: initialization: subversion <description></description> <version control></version control> <version>1.0.0.0.2</version> <version control></version control> (filenamemacro=getstisy.s) [<Program>] [<Research>] [kn, ri, tu, 31-12-2024 18:06:19]
...
// library: file: update: version: control: subversion: save: create <description>CRUD</description> <version control></version control> <version>1.0.0.0.159</version> <version control></version control> (filenamemacro=updafisc.s) [<Program>] [<Research>] [kn, zoe, mo, 20-11-2000 14:31:57]
PROC PROCFileUpdateVersionControlSubversionSaveCreateCurrent( STRING fileVersionControlDirectoryWorkingInS, STRING fileVersionControlDirectoryRepositoryS, STRING revisionChangeInformationS, INTEGER B1 )
 ...
 // e.g.  STRING s2[255] = FNStringGetDirectoryVersionControlSubversionRepositoryS()
 // e.g.  //
 ...
 // e.g.  STRING s3[255] = ""
 // e.g.  //
 // e.g.  // STRING s4[255] = "C:\TEMP\the-semware-editor-tse-code\TRUNK\" // old [kn, ri, th, 12-02-2026 19:05:43]
 // e.g.  STRING s4[255] = "G:\VERSIONCONTROL\SUBVERSION\the-semware-editor-tse-code\TRUNK\" // new [kn, ri, th, 12-02-2026 19:10:59]
 // e.g.  //
 ...
 // e.g.   s1 = Trim( GetText( 1, MAXSTRINGLEN ) )
 // e.g.  ELSE
 ...
 //
 STRING fileNameCygwinWorkingS[255] = ""
 STRING fileNameCygwinRepositoryS[255] = ""
 //
 // If you should move your repository
 // then
 // cd to that working directory
 // and
 // svn relocate <yournewrepositorydirectory>
 //
 // E.g.
 //
 //  g:
 //  cd \/versioncontrol\subversion\W1
 //  svn relocate file:///cygdrive/g/versioncontrol/subversion/R1
 //
 ...
 //
 fileNameCygwinWorkingS = FNStringGetFileNameMicrosoftwindowsToCygwinS( fileVersionControlDirectoryWorkingS )
 fileNameCygwinRepositoryS = FNStringGetFileNameMicrosoftwindowsToCygwinS( fileVersionControlDirectoryRepositoryS )
 //
 IF ( NOT ( FileExists( Format( AddTrailingSlash( fileVersionControlDirectoryRepositoryS ), "*.*" ) ) ) )
  ...
  // CopyToWinclip( Format( "c: && cd \TEMP\ && svnadmin create R1 && md T1 && svn import T1 file:///cygdrive/c/temp/R1 -m", " ", '"', revisionChangeInformationS, '"' ) ) // old [kn, ri, tu, 30-12-2025 19:45:02]
  CopyToWinclip( Format( "c: && cd \TEMP\ && svnadmin create R1 && md T1 && svn import T1", " ", fileNameCygwinRepositoryS, " ", "-m", " ", '"', revisionChangeInformationS, '"' ) ) // new [kn, ri, tu, 30-12-2025 19:45:05]
  //
  ...
  // Warn( "Repository not found. Please check the URL or create a new repository.", " ", "Copied to Microsoft Windows clipboard:", " ", "c: && cd \TEMP\ && svnadmin create R1 && md T1 && svn import T1 file:///cygdrive/c/temp/R1 -m", " ", '"', revisionChangeInformationS, '"' ) // new [kn, ri, su, 14-08-2022 12:08:04] // old [kn, ri, tu, 30-12-2025 19:45:58]
  Warn( "Repository not found. Please check the URL or create a new repository.", " ", "Copied to Microsoft Windows clipboard:", " ", "c: && cd \TEMP\ && svnadmin create R1 && md T1 && svn import T1", " ", fileNameCygwinRepositoryS, " ", "-m", " ", '"', revisionChangeInformationS, '"' ) // new [kn, ri, tu, 30-12-2025 19:46:05]
  //
  RETURN()
  //
 ENDIF
 ...
 IF ( NOT ( FileExists( Format( AddTrailingSlash( fileVersionControlDirectoryWorkingS ), "*.*" ) ) ) )
  //
  // CopyToWinclip( "c: && cd \TEMP\ && svn checkout file:///cygdrive/c/temp/R1 W1" ) // old [kn, ri, tu, 30-12-2025 19:47:03]
  CopyToWinclip( Format( "c: && cd \TEMP\ && svn checkout", " ", fileNameCygwinRepositoryS, " ", "W1" ) )// new [kn, ri, tu, 30-12-2025 19:47:37]
  //
  // Warn( "Working directory not found. Please check or create a new working directory. Copied to Microsoft Windows clipboard: 'c: && cd \TEMP\ && svn checkout file:///cygdrive/c/temp/R1 W1'" ) // old [kn, ri, tu, 30-12-2025 19:47:44]
  Warn( "Working directory not found. Please check or create a new working directory. Copied to Microsoft Windows clipboard: 'c: && cd \TEMP\ && svn checkout", " ", fileNameCygwinRepositoryS, " ", "W1'" ) // new [kn, ri, tu, 30-12-2025 19:47:47]
  //
  RETURN()
  //
 ENDIF
...
// library: string: get: file: name: microsoftwindows: to: cygwin <description></description> <version control></version control> <version>1.0.0.0.11</version> <version control></version control> (filenamemacro=getsttei.s) [<Program>] [<Research>] [kn, ri, tu, 30-12-2025 18:16:26]
STRING PROC FNStringGetFileNameMicrosoftwindowsToCygwinS( STRING fileNameS )
 // e.g. PROC Main()
 // e.g.  // STRING s1[255] = GetHistoryStr( _EDIT_HISTORY_, 1 ) // change this
 // e.g.  STRING s1[255] = "c:\temp\R1" // change this
 // e.g.  PROCMacroRunKeep( "setwiyde" ) // operation: set: window: warn/yesno: position: x: y: default // new
 // e.g.  IF ( NOT ( Ask( "string: get: file: name: microsoftwindows: to: cygwin = ", s1, _EDIT_HISTORY_ ) ) AND ( Length( s1 ) > 0 ) ) RETURN() ENDIF
 // e.g.  Message( FNStringGetFileNameMicrosoftwindowsToCygwinS( s1 ) ) // gives e.g. "file:///cygdrive/c/temp/R1"
 // e.g. END
 // e.g.
 // e.g. <Ctrl F12> Main()
 //
 ...
 //
 // out:
 // file:///cygdrive/c/temp/R1
 //
 // ===
 //
 // Still to implement in another function, the inverse
 //
 // in:
 // file:///cygdrive/c/temp/R1
 //
 ...
 */
 ...
 // ===
 //
 // e.g. // QuickHelp( HELPDEFFNStringGetFileNameMicrosoftwindowsToCygwinS )
 // e.g. HELPDEF HELPDEFFNStringGetFileNameMicrosoftwindowsToCygwinS
 // e.g.  title = "FNStringGetFileNameMicrosoftwindowsToCygwinS( s1 ) help" // The help's caption
 // e.g.  x = 100 // Location
 // e.g.  y = 3 // Location
 // e.g.  //
 // e.g.  // The actual help text
