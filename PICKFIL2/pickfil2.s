...
proc Main()
    integer id, ok
    string s[255]
    string silentS[8] = Lower(Trim(GetProfileStr("Settings", "silent", "false", "pickfil2.ini")))
    if silentS <> "true"
        Warn("PICKFIL2 1.0.0.0.0: Browse directories and select a file with Enter. Alt-F10 selects a drive; Esc quits.")
    endif
    PushPosition()
    id = CreateTempBuffer()
    s = ""
