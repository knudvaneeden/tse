...
FORWARD PROC PROCProgramLoadStateEditorTextFilenameTse( STRING s1 )
...
PROC Main()
 ...
 IF ( NOT ( AskFilename( "program: load: state: editor: text: filename: tse: fileNameS = ", s1, _DEFAULT_, _EDIT_HISTORY_ ) ) ) RETURN() ENDIF
 PROCProgramLoadStateEditorTextFilenameTse( s1 )
END
...
// library: program: load: state: editor: text: filename: tse <description></description> <version>1.0.0.0.6</version> <version control></version control> (filenamemacro=loadprft.s) [<Program>] [<Research>] [kn, ri, sa, 08-01-2011 19:50:17]
PROC PROCProgramLoadStateEditorTextFilenameTse( STRING fileNameS )
 // e.g. PROC Main()
 // e.g.  STRING s1[255] = "*.dat"
 // e.g.  PROCMacroRunKeep( "setwiyde" ) // operation: set: window: warn/yesno: position: x: y: default // new
 // e.g.  IF ( NOT ( AskFilename( "program: load: state: editor: text: filename: tse: fileNameS = ", s1, _DEFAULT_, _EDIT_HISTORY_ ) ) ) RETURN() ENDIF
 // e.g.  PROCProgramLoadStateEditorTextFilenameTse( s1 )
 // e.g. END
 // e.g.
 // e.g. <F12> Main()
 //
 ExecMacro( Format( "state", " ", "-r", " ", "-f", fileNameS ) )
 //
END
