...
FORWARD STRING PROC FNStringGetCrossPlatFormSToFileNameS( STRING s1 )
...
FORWARD STRING PROC FNStringGetLinuxNonWslToLinuxWslFileNameS( STRING s1 )
FORWARD STRING PROC FNStringGetLinuxNonWslToMicrosoftWindowsFileNameS( STRING s1 )
FORWARD STRING PROC FNStringGetLinuxWslToLinuxNonWslFileNameS( STRING s1 )
FORWARD STRING PROC FNStringGetLinuxWslToMicrosoftWindowsFileNameS( STRING s1 )
 ...
 */
 ...
 // Tested on:
 //
 //  1. TSE for Microsoft Windows version 4.50.19 and version 4.50 RC24
 //
 //  2. TSE for Linux WSL version 4.50.14 (Ubuntu)
 //
 //  3. TSE for Linux non-WSL version 4.50.19 (Ubuntu)
 //
 // ===
 //
 ...
 PushBlock()
 //
 // copy the marked block of TSE program source code
 //
 Copy()
 //
 // A temporary filename to use to run the TSE program source code
 //
 fileNameS = "c:\temp\ddd.s" // change this
 //
 // convert this Microsoft Windows filename to the filename to use on the current operating system
 //
 fileNameS = FNStringGetCrossPlatFormSToFileNameS( fileNameS )
 //
 ...
 //
 // compile and execute current filename macro
 //
 ExecMacro( "compile" )
 //
 // Not necessary as handled already by this 'compile' TSE macro.
 //
...
// library: string: get: cross: plat: form: s: to: file: name <description></description> <version control></version control> <version>1.0.0.0.8</version> <version control></version control> (filenamemacro=getstfnn.s) [<Program>] [<Research>] [kn, ri, sa, 07-02-2026 15:26:48]
STRING PROC FNStringGetCrossPlatFormSToFileNameS( STRING fileNameInS )
 // e.g. PROC Main()
 ...
 // e.g.   Warn( FNStringGetCrossPlatFormSToFileNameS( s1 ) ) // gives e.g. "c:\temp\ddd.s"
 // e.g.  UNTIL FALSE
 // e.g. END
 // e.g.
 // e.g. <Ctrl F12> Main()
 ...
 */
 ...
 // ===
 //
 // e.g. // QuickHelp( HELPDEFFNStringGetCrossPlatFormSToFileName )
 // e.g. HELPDEF HELPDEFFNStringGetCrossPlatFormSToFileName
 // e.g.  title = "FNStringGetCrossPlatFormSToFileName( s1 ) help" // The help's caption
 // e.g.  x = 100 // Location
 // e.g.  y = 3 // Location
 // e.g.  //
 // e.g.  // The actual help text
 ...
 STRING fileNameS[255] = ""
 //
 // If only filename and optionally a file extension do not change the filename and keep it unchanged
 //
 IF NOT ( StrFind( "/", fileNameInS, "" ) ) AND NOT ( StrFind( "\", fileNameInS, "" ) )
  //
  fileNameS = fileNameInS
  //
 ELSEIF FNProgramGetOperatingSystemLinuxNonWslB()
  //
  IF ( LeftStr( fileNameInS, Length( "/home/" ) ) == "/home/" )
   //
   fileNameS = fileNameInS
   //
  ELSEIF ( LeftStr( fileNameInS, Length( "/mnt/" ) ) == "/mnt/" )
    //
    fileNameS = FNStringGetLinuxWslToLinuxNonWslFileNameS( fileNameInS )
   //
  ELSEIF StrFind( "[A-Za-z]:", fileNameInS, "ix" ) OR StrFind( "\", fileNameInS, "" )
   //
   fileNameS = FNStringGetMicrosoftWindowsToLinuxNonWslFileNameS( fileNameInS )
  //
  ELSEIF StrFind( "/", fileNameInS, "" )
   //
   fileNameS = fileNameInS
   //
  ELSE
   //
   Warn( fileNameInS, ":", " ", "The filename format is not recognized. Please check." )
   //
   fileNameS = fileNameInS
  //
  ENDIF
 ...
 ELSEIF FNProgramGetOperatingSystemLinuxWslB()
  ...
  IF ( LeftStr( fileNameInS, Length( "/home/" ) ) == "/home/" )
   //
   fileNameS = FNStringGetLinuxNonWslToLinuxWslFileNameS( fileNameInS )
   //
  ELSEIF ( LeftStr( fileNameInS, Length( "/mnt/" ) ) == "/mnt/" )
   //
   fileNameS = fileNameInS
 ...
 ELSEIF FNProgramGetOperatingSystemMicrosoftWindowsB()
  //
  IF ( LeftStr( fileNameInS, Length( "/home/" ) ) == "/home/" ) OR ( LeftStr( fileNameInS, Length( "~/" ) ) == "~/" )
   //
   fileNameS = FNStringGetLinuxNonWslToMicrosoftWindowsFileNameS( fileNameInS )
   //
  ELSEIF ( LeftStr( fileNameInS, Length( "/mnt/" ) ) == "/mnt/" )
    //
    fileNameS = FNStringGetLinuxWslToMicrosoftWindowsFileNameS( fileNameInS )
   //
  ELSEIF StrFind( "[A-Za-z]:", fileNameInS, "ix" ) OR StrFind( "\", fileNameInS, "" )
   //
   fileNameS = fileNameInS
  ...
  ELSEIF StrFind( "/", fileNameInS, "" )
   //
   fileNameS = StrReplace( "/", fileNameInS, "\", "" )
   //
  ELSE
   //
   Warn( fileNameInS, ":", " ", "The filename format is not recognized. Please check." )
...
// library: string: get: linux: wsl: to: linux: non: wsl: file: name <description></description> <version control></version control> <version>1.0.0.0.3</version> <version control></version control> (filenamemacro=getstfnr.s) [<Program>] [<Research>] [kn, ri, sa, 07-02-2026 15:43:38]
STRING PROC FNStringGetLinuxWslToLinuxNonWslFileNameS( STRING fileNameS )
 // e.g. PROC Main()
 // e.g.  // Define a string array with a default Microsoft Windows path, adjust as necessary.
 // e.g.  STRING s1[255] = "/mnt/c/temp/tse_linux/tse/ddd.s" // change this // Parameter: fileNameS - The original Microsoft Windows file path as a string.
 // e.g.  PROCMacroRunKeep( "setwiyde" ) // operation: set: window: warn/yesno: position: x: y: default // new
 ...
 // e.g.  IF ( NOT ( Ask( "string: get: linux: wsl: to: linux: non: wsl: file: name: fileNameS = ", s1, _EDIT_HISTORY_ ) ) AND ( Length( s1 ) > 0 ) ) RETURN() ENDIF
 // e.g.  // Convert the Linux WSL path to a Microsoft Windows path.
 // e.g.  s1 = FNStringGetLinuxWslToLinuxNonWslFileNameS( s1 ) // gives e.g. "c:\temp\ddd.s"
 ...
 // e.g.  Warn( s1, "has been copied to the Microsoft Windows ClipBoard" ) // from input e.g. '/mnt/c/temp/foobar.txt' gives e.g. output '/home/knudvaneeden/c/temp/foobar.txt'
 ...
 // ===
 //
 // Use case = Converts a Linux Non-WSL file path to a Linux WSL file path
 //
 // ===
 //
 // ===
 ...
 */
 ...
 // ===
 //
 // e.g. // QuickHelp( HELPDEFFNStringGetLinuxNonWslToLinuxWslFileNameS )
 // e.g. HELPDEF HELPDEFFNStringGetLinuxNonWslToLinuxWslFileNameS
 // e.g.  title = "FNStringGetLinuxNonWslToLinuxWslFileNameS( s1 ) help" // The help's caption
 // e.g.  x = 100 // Location
 // e.g.  y = 3 // Location
 // e.g.  //
 // e.g.  // The actual help text
 ...
 s = fileNameS
 //
 // Remove the prefix to the path '/mnt/' which is a typical mount point for WSL.
 s = StrReplace( "/mnt/", s, "" )
 //
 // Add the prefix '/home/' and also the username followed by a slashh '/' in the front
...
// library: string: get: linux: non: wsl: to: linux: wsl: file: name <description></description> <version control></version control> <version>1.0.0.0.4</version> <version control></version control> (filenamemacro=getstfnq.s) [<Program>] [<Research>] [kn, ri, sa, 07-02-2026 15:28:36]
STRING PROC FNStringGetLinuxNonWslToLinuxWslFileNameS( STRING fileNameS )
 // e.g. PROC Main()
 // e.g.  // Define a string array with a default Microsoft Windows path, adjust as necessary.
 // e.g.  STRING s1[255] = "/home/knudvaneeden/c/temp/tse_linux/tse/ddd.s" // change this // Parameter: fileNameS - The original Microsoft Windows file path as a string.
 // e.g.  PROCMacroRunKeep( "setwiyde" ) // operation: set: window: warn/yesno: position: x: y: default // new
 // e.g.  // Prompt the user to input a file path, with edit history enabled.
 // e.g.  // Checks if user cancelled the prompt (input is empty) and returns if true.
 // e.g.  IF ( NOT ( Ask( "string: get: linux: non: wsl: to: microsoft: windows: file: name: fileNameS = ", s1, _EDIT_HISTORY_ ) ) AND ( Length( s1 ) > 0 ) ) RETURN() ENDIF
 // e.g.  // Convert the Linux non-WSL path to a Microsoft Windows path.
 // e.g.  s1 = FNStringGetLinuxNonWslToLinuxWslFileNameS( s1 ) // gives e.g. "/mnt/c/temp/ddd.s"
 // e.g.  // Display a message that the path has been converted and copied to the clipboard.
 // e.g.  // Message( "cd", " ", s1, " ", "has been copied to the Microsoft Windows ClipBoard" )
 // e.g.  Warn( s1, "has been copied to the Microsoft Windows ClipBoard" )
 // e.g.  // Copy the Linux path command to the Microsoft Windows clipboard.
 ...
 */
 ...
 //
 // e.g. // QuickHelp( HELPDEFFNStringGetLinuxNonWslToLinuxWslFileNameS )
 // e.g. HELPDEF HELPDEFFNStringGetLinuxWslToLinuxWslFileNameS
 // e.g.  title = "FNStringGetLinuxWslToLinuxWslFileNameS( s1 ) help" // The help's caption
 // e.g.  x = 100 // Location
 // e.g.  y = 3 // Location
 // e.g.  //
 // e.g.  // The actual help text
 ...
 s = fileNameS
 //
 // Remove the prefix to the path '/home/' and also the username in the front
 s = StrReplace( Format( "/home/", FNStringGetUserNameLinuxNonWslS(), "/" ), s, "" )
 //
 // Add '/mnt/' in front
 s = Format( "/mnt/", s )
 //
...
// library: string: get: linux: non: wsl: to: microsoft: windows: file: name <description></description> <version control></version control> <version>1.0.0.0.7</version> <version control></version control> (filenamemacro=getstfnm.s) [<Program>] [<Research>] [kn, ri, we, 15-10-2025 20:00:04]
STRING PROC FNStringGetLinuxNonWslToMicrosoftWindowsFileNameS( STRING fileNameS )
 // e.g. PROC Main()
 // e.g.  // Define a string array with a default Microsoft Windows path, adjust as necessary.
 // e.g.  // STRING s1[255] = "/home/knudvaneeden/c/temp/tse_linux/tse/ddd.s" // change this // Parameter: fileNameS - The original Microsoft Windows file path as a string.
 // e.g.  STRING s1[255] = "~/c/temp/tse_linux/tse/ddd.s" // change this // Parameter: fileNameS - The original Microsoft Windows file path as a string.
 // e.g.  PROCMacroRunKeep( "setwiyde" ) // operation: set: window: warn/yesno: position: x: y: default // new
 // e.g.  // Prompt the user to input a file path, with edit history enabled.
 // e.g.  // Checks if user cancelled the prompt (input is empty) and returns if true.
 // e.g.  IF ( NOT ( Ask( "string: get: linux: non: wsl: to: microsoft: windows: file: name: fileNameS = ", s1, _EDIT_HISTORY_ ) ) AND ( Length( s1 ) > 0 ) ) RETURN() ENDIF
 // e.g.  // Convert the Linux non-WSL path to a Microsoft Windows path.
 // e.g.  s1 = FNStringGetLinuxNonWslToMicrosoftWindowsFileNameS( s1 ) // gives e.g. "c:\temp\ddd.s"
 // e.g.  // Display a message that the path has been converted and copied to the clipboard.
 // e.g.  // Message( "cd", " ", s1, " ", "has been copied to the Microsoft Windows ClipBoard" )
 // e.g.  Warn( s1, "has been copied to the Microsoft Windows ClipBoard" )
 // e.g.  // Copy the Linux path command to the Microsoft Windows clipboard.
 ...
 // ===
 //
 // Use case = Converts a Linux Non-WSL file path to a Microsoft Windows file path
 //
 // ===
 //
 // ===
 ...
 */
 ...
 // ===
 //
 // e.g. // QuickHelp( HELPDEFFNStringGetLinuxWslToMicrosoftWindowsFileNameS )
 // e.g. HELPDEF HELPDEFFNStringGetLinuxWslToMicrosoftWindowsFileNameS
 // e.g.  title = "FNStringGetLinuxWslToMicrosoftWindowsFileNameS( s1 ) help" // The help's caption
 // e.g.  x = 100 // Location
 // e.g.  y = 3 // Location
 // e.g.  //
 // e.g.  // The actual help text
 ...
 s = StrReplace( Format( "~/", "" ), s, "" )
 //
 // Replace the slash '/' used in Linux filenames with the backslash '\' as used in Microsoft Windows
 s = StrReplace( "/", s, "\", "" )
 //
 // Insert a colon ":" at the second character from the left after the first character (which is the drive letter e.g. 'c' thus)
 s = Format( LeftStr( s, 1 ), ":", RightStr( s, Length( s ) - 1 ) )
 //
 // Return the converted path
 //
 RETURN( s )
...
// library: string: get: linux: wsl: to: microsoft: windows: file: name <description></description> <version control></version control> <version>1.0.0.0.6</version> <version control></version control> (filenamemacro=getstfnj.s) [<Program>] [<Research>] [kn, ri, th, 05-12-2024 14:40:55]
STRING PROC FNStringGetLinuxWslToMicrosoftWindowsFileNameS( STRING fileNameS )
 // e.g. PROC Main()
 // e.g.  // Define a string array with a default Microsoft Windows path, adjust as necessary.
 // e.g.  STRING s1[255] = "/mnt/c/temp/tse_linux/tse/ddd.s" // change this // Parameter: fileNameS - The original Microsoft Windows file path as a string.
 // e.g.  PROCMacroRunKeep( "setwiyde" ) // operation: set: window: warn/yesno: position: x: y: default // new
 // e.g.  // Prompt the user to input a file path, with edit history enabled.
 // e.g.  // Checks if user cancelled the prompt (input is empty) and returns if true.
 // e.g.  IF ( NOT ( Ask( "string: get: linux: wsl: to: microsoft: windows: file: name: fileNameS = ", s1, _EDIT_HISTORY_ ) ) AND ( Length( s1 ) > 0 ) ) RETURN() ENDIF
 // e.g.  // Convert the Linux WSL path to a Microsoft Windows path.
 // e.g.  s1 = FNStringGetLinuxWslToMicrosoftWindowsFileNameS( s1 ) // gives e.g. "c:\temp\ddd.s"
 // e.g.  // Display a message that the path has been converted and copied to the clipboard.
 // e.g.  // Message( "cd", " ", s1, " ", "has been copied to the Microsoft Windows ClipBoard" )
 // e.g.  Warn( s1, "has been copied to the Microsoft Windows ClipBoard" ) // from input e.g. '/mnt/c/temp/foobar.txt' gives e.g. output 'c:\temp\foobar.txt'
 // e.g.  // Copy the Linux path command to the Microsoft Windows clipboard.
 // e.g.  // CopyToWinClip( Format( "cd", " ", s1 ) )
 // e.g.  CopyToWinClip( Format( s1 ) )
 // e.g. END
 ...
 // ===
 //
 // Use case = Converts a Linux WSL file path to a Microsoft Windows file path
 //
 // ===
 //
 // ===
